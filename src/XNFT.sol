...
contract XNFT is ERC721, ERC721URIStorage, ERC721Burnable, IAny2EVMMessageReceiver, ReentrancyGuard, OwnerIsCreator {
    ...
    //@note can only be called from the Arbitrum chain => associated func cannot be called if contract is deployed to different chain
    ...
    //mint is only available on Arbitrum => onlyOnArbitrumSepolia
    ...
    //enable cross chain and corresponding XNFT
    ...
    function crossChainTransferFrom(
        ...
        //@note receiver: XNFT contract on target chain
    ...
    //@note only router can call this & only msg from enabled chains & senders are allowed
        ...
        onlyEnabledSender(message.sourceChainSelector, abi.decode(message.sender, (address)))
    {
        ...
        (address from, address to, uint256 tokenId, string memory tokenUri) = abi.decode(
            message.data,
    ...
    function supportsInterface(bytes4 interfaceId) public view override(ERC721, ERC721URIStorage) returns (bool) {
        //@note specify interfaces from inherited contracts => || super.supportsInterface(interfaceId)
